...
def initialize_structures(PHASES_INFO, two_theta_range, step):
    ...
    for phase_info in PHASES_INFO:
        # print(f"Add ObjCryst phase {phase_info['Name']}")
...
def generate_combined_diffraction_pattern(sample_info, PHASES_INFO, two_theta_range, step, weights):
    ...
    quarter_max_intensity = max_intensity * random.uniform(0.03, 0.25)
    ...
    dev_int = random.uniform(0.0002, 0.001)
    ...
    background += (-1 * np.min(background)) ###+ bkg_weight * max_profile_int
