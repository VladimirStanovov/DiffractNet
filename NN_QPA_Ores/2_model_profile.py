...
def main():
    ...
    if experimental_data:
        ...
        root, _ = os.path.splitext(first_experiment_filename)
        conc_filename = experiment_folder + root + '.conc'

        # чтение массива чисел
        read_wghts = []
        with open(conc_filename, 'r', encoding='utf-8') as f:
            for line in f:
                line = line.strip()
                if line:  # пропускаем пустые строки
                    read_wghts.append(float(line))
