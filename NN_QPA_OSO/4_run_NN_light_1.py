...
class DiffractionNet(nn.Module):
    def __init__(self, input_size, num_phases):
        ...
        # Сверточные слои: уменьшено количество ядер (~1.5-2 раза)
        self.conv_layers = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=48, kernel_size=7, stride=1, padding=3),  # Было 64
            nn.BatchNorm1d(48),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2),
            nn.Conv1d(in_channels=48, out_channels=96, kernel_size=5, stride=1, padding=2), # Было 128
            nn.BatchNorm1d(96),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2),
            nn.Conv1d(in_channels=96, out_channels=192, kernel_size=3, stride=1, padding=1), # Было 256
            nn.BatchNorm1d(192),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2)
        )
        ...
        # Трансформер: незначительно уменьшена скрытая размерность feed-forward слоя
        self.transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=192, nhead=8, dim_feedforward=384, dropout=0.1, batch_first=True), # d_model=192, dim_feedforward в 2 раза больше
            num_layers=2  # Без изменений
        )
        
        # Полносвязные слои: уменьшена ширина (~2 раза)
        self.fc_layers = nn.Sequential(
            nn.Linear(192 * reduced_size, 512),  # Было 1024
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Dropout(0.1),
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1)  # Критично: (batch, seq_len=reduced_size, features=d_model)
