...
class DiffractionNet(nn.Module):
    def __init__(self, input_size, num_phases):
        ...
        self.transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=128, nhead=8, dim_feedforward=256, dropout=0.1, batch_first=True), # d_model и dim_feedforward уменьшены
...
train_labels = np.array([find_index_of_max(out) for out in train_outputs]).astype(int)
