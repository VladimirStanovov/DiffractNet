...
def load_batch_inp_test_file(file_name):
    if os.path.exists(file_name):
        with open(file_name, 'rb') as f:
            while True:
                data = f.read(struct.calcsize(f'{input_size}f'))
                if not data:
                    break
                inp = struct.unpack(f'{input_size}f', data)
                inp_test_data.append(np.array(inp))


def load_batch_out_test_file(file_name):
    if os.path.exists(file_name):
        with open(file_name, 'rb') as f:
            while True:
                data = f.read(struct.calcsize(f'{num_phases}f'))
                if not data:
                    break
                out = struct.unpack(f'{num_phases}f', data)
                out_test_data.append(np.array(out))
...
inp_test_files = glob.glob(f"{difr_dir}batch_inp_{xml_test_name}_{num_phases}ph_p*.bin")
out_test_files = glob.glob(f"{difr_dir}batch_out_{xml_test_name}_{num_phases}ph_p*.bin")
...
for file in inp_test_files:
    load_batch_inp_test_file(file)

for file in out_test_files:
    load_batch_out_test_file(file)

print(f"Размер массива out_test_data: {len(out_test_data)}\n")

if len(out_data) < 1:
    print("Внимание - нет данных для обучения !")

if len(out_test_data) < 1:
    print("Внимание - нет данных для тестирования !")
...
train_dataset = CandleDataset(inp_data, out_data)
test_dataset = CandleDataset(inp_test_data, out_test_data)
...
train_labels = np.array([find_index_of_max(out) for out in out_data])
