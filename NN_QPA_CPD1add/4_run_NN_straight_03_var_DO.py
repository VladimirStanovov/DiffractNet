...
class DiffractionNet(nn.Module):
    def __init__(self, input_size, num_phases, dropout_prob=0.1):
        ...
        # Создаем свой TransformerEncoder с изменяемым dropout
        ...
        # Сохраняем dropout_prob для дальнейшего использования
        self.current_dropout = dropout_prob
        
    def set_dropout(self, dropout_prob):
        """Устанавливает значение dropout для всех слоев"""
        self.current_dropout = dropout_prob
        
        # Создаем новый TransformerEncoderLayer с обновленным dropout
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=256, 
            nhead=8, 
            dim_feedforward=512, 
        ...
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=2)
        self.transformer.to(device)  # Переносим на устройство
        
        # Обновляем dropout в полносвязных слоях
...
# Создание экземпляра модели с начальным dropout=0.2
initial_dropout = 0.2
model = DiffractionNet(input_size, num_phases, dropout_prob=initial_dropout).to(device)
...
if os.path.exists(wghts_fname):
    ...
    # При загрузке весов нужно установить начальный dropout
    model.set_dropout(initial_dropout)
else:
...
with open(csv_res_fname, mode='w', newline='') as file:
    ...
    header = ['Epoch', 'Train Loss', 'Test Loss', 'Exp_Diff', 'Dropout'] + \
...
for epoch in range(num_epochs):
    # Вычисляем текущий dropout
    if epoch < 90:
        # Линейное уменьшение от 0.2 до 0.0 за 90 эпох
        current_dropout = 0.2 * (1 - epoch / 90)
    else:
        # Последние 10 эпох с нулевым dropout
        current_dropout = 0.0
    ...
    for inputs, outputs in train_dataloader:
        ...
        if iters % 500 == 0:
            print(f"Iteration {iters}, Dropout: {current_dropout:.3f}")
    ...
    print(
        f'Epoch [{epoch + 1}/{num_epochs}], Dropout: {current_dropout:.3f}, learn_rate: {learn_rate}, Train Loss: {100 * np.sqrt(train_loss):.5f}, Test Loss: {100 * np.sqrt(test_loss):.5f}')
    ...
    with open(csv_res_fname, mode='a', newline='') as file:
        writer = csv.writer(file, delimiter=';')
        row = [epoch + 1, 100 * np.sqrt(train_loss), 100 * np.sqrt(test_loss), total_exp_diff, current_dropout] + \
