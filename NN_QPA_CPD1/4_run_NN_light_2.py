...
class DiffractionNet(nn.Module):
    def __init__(self, input_size, num_phases):
        ...
        # Сверточные слои: ОРИГИНАЛЬНЫЕ (самая важная часть для признаков)
        ...
        # Трансформер: ТОЛЬКО ОДИН СЛОЙ. Это основное упрощение.
        self.transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=256, nhead=8, dim_feedforward=512, dropout=0.1, batch_first=True),
            num_layers=1  # Было 2
        )
        
        # Полносвязные слои: небольшое уменьшение
        self.fc_layers = nn.Sequential(
            nn.Linear(256 * reduced_size, 768),  # Было 1024
            nn.BatchNorm1d(768),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(768, num_phases)
        )
    
    def forward(self, x):
        x = self.conv_layers(x)
        x = x.permute(0, 2, 1)  # Порядок размерностей сохранён!
